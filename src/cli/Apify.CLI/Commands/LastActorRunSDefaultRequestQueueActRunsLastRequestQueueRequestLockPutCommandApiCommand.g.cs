...
internal static partial class LastActorRunSDefaultRequestQueueActRunsLastRequestQueueRequestLockPutCommandApiCommand
{
...
",
    ...
    {
        var command = new Command(@"act-runs-last-request-queue-request-lock-put", @"Prolong lock on request in last run's default request queue
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.LastActorRunSDefaultRequestQueue.ActRunsLastRequestQueueRequestLockPutAsync(
                                    actorId: actorId,
                                    requestId: requestId,
                                    status: status,
                                    origin: origin,
