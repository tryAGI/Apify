...
internal static partial class LastActorRunSDefaultKeyValueStoreActorRunsLastKeyValueStoreRecordPostCommandApiCommand
{
    ...
    private static Option<global::Apify.ActorRunsLastKeyValueStoreRecordPostContentEncoding?> ContentEncoding { get; } = new(
    ...
    {
        var command = new Command(@"actor-runs-last-key-value-store-record-post", @"Store record in last run's default store (POST)
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.LastActorRunSDefaultKeyValueStore.ActorRunsLastKeyValueStoreRecordPostAsync(
                                    actorId: actorId,
                                    recordKey: recordKey,
                                    status: status,
                                    origin: origin,
