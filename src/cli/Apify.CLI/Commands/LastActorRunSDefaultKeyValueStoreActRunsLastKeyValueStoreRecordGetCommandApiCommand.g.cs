...
internal static partial class LastActorRunSDefaultKeyValueStoreActRunsLastKeyValueStoreRecordGetCommandApiCommand
{
...
");
    ...
    {
        var command = new Command(@"act-runs-last-key-value-store-record-get", @"Get last run's default store's record
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.LastActorRunSDefaultKeyValueStore.ActRunsLastKeyValueStoreRecordGetAsync(
                                    actorId: actorId,
                                    recordKey: recordKey,
                                    status: status,
                                    origin: origin,
